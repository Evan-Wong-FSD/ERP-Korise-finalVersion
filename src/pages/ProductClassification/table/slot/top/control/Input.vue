    ...
    >
      <div v-for="(elem, index) of inputBoxs" :key="index" class="control-input">
        <q-select
          ...
          outlined
          clearable
          :use-input="elem.name !== 'productPartNumber' && elem.name !== 'productSubclassPartNumber'"
          input-debounce="500"
          :new-value-mode="elem.name !== 'productPartNumber' && elem.name !== 'productSubclassPartNumber' ? 'add-unique' : undefined"
          :ref="elem.name"
          :label="elem.label"
  ...
  methods: {
    ...
    filterSelectOptions (select, index, typeIn, update, abort) {
      ...
      if (typeIn.length < 2) {
        ...
        setTimeout(() => {
          ProductClassificationAPI.get('/api/filterSelectOptions', { params: { inputBoxs: JSON.stringify(inputBoxs), select: JSON.stringify(select), typeIn } }).then(res => {
            update(() => {
    ...
    selectInput (select, elemIndex, elemInput) {
      const inputCouples = [['taxIdNumber', 'firm'], ['productClass', 'productPartNumber'], ['productSubclass', 'productSubclassPartNumber']]
      ...
      if (select.name === 'productClass' && select.value === '其他收入') {
        ...
        const productSubclassPartNumber = this.inputBoxs.find(elem => elem.name === 'productSubclassPartNumber')
        productSubclass.value = null
        productSubclassPartNumber.value = '000'
        this.updateControlInputsValue({ name: productSubclass.name, input: productSubclass.value })
        this.updateControlInputsValue({ name: productSubclassPartNumber.name, input: productSubclassPartNumber.value })
      }

      if (coupleFilled(this.inputBoxs)) return

      const params = { inputBoxs: JSON.stringify(this.inputBoxs), targetLabel: targetLabel() }
      ProductClassificationAPI.get('/api/obtainControlInputs', { params }).then(res => {
      ...
      function targetLabel () {
        ...
        if (select.name === 'productPartNumber') return '產品種類'
        if (select.name === 'productSubclass') return '材質料號'
        if (select.name === 'productSubclassPartNumber') return '產品材質'
      }
    },
    onDisable (name) {
      const input = (name) => this.inputBoxs.find(elem => elem.name === name)
      if (name === 'productClass' || name === 'productPartNumber') {
        return !(input('taxIdNumber').value && input('firm').value)
      } else if (name === 'productSubclass' || name === 'productSubclassPartNumber') {
        const productSubclass = input('productClass')
        return productSubclass.value === '其他收入' || !(productSubclass.value && input('productPartNumber').value)
      }
    },
    selectRules (select, input) {
      ...
      if (name === 'productPartNumber') errorMessage = RegExp('^K[0-9]{3}$').test(input) || `${label}輸入錯誤`
      if (name === 'productSubclassPartNumber') errorMessage = RegExp('^[0-9]{2,3}$').test(input) || `${label}輸入錯誤`
    ...
    onSubmit () {
      ProductClassificationAPI.post('/api/createProductClass', { inputBoxs: this.inputBoxs }).then(async res => {
